...
export function generateStage(params: { students: StudentRecord[]; staff: StaffRecord[]; eligibleStudentIds: Set<string>; presentStaffIds: Set<string>; histories: History[]; servicePart: 1 | 2; singerSlots: number; choirSlots: number; leaderType: "student" | "staff"; leaderId: string; usedStaffIds: Set<string>; missedPreviousWeekIds?: Set<string> }) {
  ...
  const staffPool = params.staff.filter((item) => params.presentStaffIds.has(item.id) && item.id !== params.leaderId)
  ...
  const continuityRisks = studentPool.filter((student) => params.missedPreviousWeekIds?.has(student.id) && !assignedStudentIds.has(student.id));
