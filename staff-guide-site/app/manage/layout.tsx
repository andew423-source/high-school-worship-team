...
export default async function ManageLayout({ children }: { children: ReactNode }) {
  ...
  return <div className="manage-page">
    <header className="manage-topbar">
      <Link className="manage-brand" href={user.role === "admin" ? "/" : "/manage"}><span>W</span><div><b>고등부 찬양팀</b><small>운영 센터</small></div></Link>
      <nav className="manage-nav" aria-label="운영 메뉴"><Link href="/manage">{user.role === "admin" ? "학기 홈" : "주차 홈"}</Link>{user.role === "admin" && <Link href="/">스탭 가이드</Link>}</nav>
