...
export async function GET(request: Request) {
  ...
  const [meetings, groups, students, rows] = await Promise.all([
    all("SELECT * FROM meetings WHERE term_id=? ORDER BY meeting_date", [termId]),
    all(`SELECT g.*,COALESCE((SELECT group_concat(s.name, ', ') FROM group_staff gs JOIN staff s ON s.id=gs.staff_id WHERE gs.group_id=g.id),'') staff_names FROM groups g WHERE g.id IN (${marks}) ORDER BY g.sort_order`, groupIds),
