...
export async function PUT(request: Request) {
  ...
  if (body.action === "save_record" && body.record) {
    ...
    } else await run("INSERT INTO staff (id,name,gender,email,duty,can_sing,can_lead_group,preferred_service,active,notes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,gender=excluded.gender,email=excluded.email,duty=excluded.duty,can_sing=excluded.can_sing,can_lead_group=excluded.can_lead_group,preferred_service=excluded.preferred_service,active=excluded.active,notes=excluded.notes,updated_at=excluded.updated_at",
      [id, String(record.name).trim(), record.gender || null, record.email || null, record.duty || null, Number(Boolean(record.canSing ?? record.can_sing)), Number(Boolean(record.canLeadGroup ?? record.can_lead_group)), Number(record.preferredService ?? record.preferred_service) || null, Number(record.active ?? 1), record.notes || null, timestamp, timestamp]);
  ...
  for (const row of validRows) {
    ...
    } else await run("INSERT INTO staff (id,name,gender,email,duty,can_sing,can_lead_group,preferred_service,active,notes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,1,?,?,?)",
      [createId("staff"), row.name, row.gender || null, row.email || null, row.duty || null, Number(Boolean(row.canSing)), Number(Boolean(row.canLeadGroup)), row.preferredService || null, row.notes || null, timestamp, timestamp]);
  }
