...
export async function PUT(request: Request) {
  ...
  if (body.action === "save_record" && body.record) {
    ...
    if (body.kind === "students") await run("INSERT INTO students (id,name,grade,gender,service_part,worship_team,is_student_leader,active,notes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,grade=excluded.grade,gender=excluded.gender,service_part=excluded.service_part,worship_team=excluded.worship_team,is_student_leader=excluded.is_student_leader,active=excluded.active,notes=excluded.notes,updated_at=excluded.updated_at",
  ...
  for (const row of validRows) {
    if (body.kind === "students") await run("INSERT INTO students (id,name,grade,gender,service_part,worship_team,is_student_leader,active,notes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,1,?,?,?)",
      [createId("student"), row.name, row.grade || null, row.gender || null, row.servicePart, row.worshipTeam || null, Number(Boolean(row.isStudentLeader)), row.notes || null, timestamp, timestamp]);
