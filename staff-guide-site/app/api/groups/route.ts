...
export async function POST(request: Request) {
  ...
  if (body.action === "configure") {
    ...
    const [students, groups, ruleData] = await Promise.all([all<StudentRecord>("SELECT s.id,s.name,s.notes,ts.grade,ts.gender,ts.service_part,ts.worship_team,ts.is_student_leader,ts.active FROM term_students ts JOIN students s ON s.id=ts.student_id WHERE ts.term_id=? AND ts.active=1", [body.termId]), all<GroupRecord>("SELECT * FROM groups WHERE term_id=?", [body.termId]), loadRules(body.termId)]); const settings = await loadSettings(body.termId, groups, ruleData.rules);
    const result = autoAssignGroups(students, groups, ruleData.constraints, Number(body.seed) || 1, settings); if (result.error) return jsonError(result.error, 409); const db = getD1(); const statements = [db.prepare("DELETE FROM group_members WHERE term_id=?").bind(body.termId)];
    for (const assignment of result.assignments) statements.push(db.prepare("INSERT INTO group_members (id,term_id,group_id,student_id,assigned_by,created_at) VALUES (?,?,?,?,?,?)").bind(createId("member"), body.termId, assignment.groupId, assignment.studentId, user.id, timestamp)); await db.batch(statements);
    await audit(user.id, "auto_assign", "groups", body.termId, null, { seed: body.seed, score: result.score }); return Response.json({ ok: true, score: result.score, warnings: result.warnings });
