...
export async function GET(request: Request) {
  ...
  const [groups, members, assignedStaff, constraints, students, staff] = await Promise.all([
    ...
    all("SELECT gm.*,s.name,s.grade,s.gender,s.service_part FROM group_members gm JOIN students s ON s.id=gm.student_id WHERE gm.term_id=?", [termId]),
...
export async function POST(request: Request) {
  ...
  if (body.action === "configure") {
    ...
    const [students, groups, constraints, staff] = await Promise.all([all<StudentRecord>("SELECT * FROM students WHERE active=1"), all<GroupRecord>("SELECT * FROM groups WHERE term_id=?", [body.termId]), all<GroupConstraint>("SELECT * FROM group_constraints WHERE term_id=?", [body.termId]), all<StaffRecord>("SELECT * FROM staff WHERE active=1 AND can_lead_group=1 ORDER BY name")]);
    ...
    const [unassigned, overCapacity, staffShortage] = await Promise.all([
      all("SELECT id FROM students WHERE active=1 AND id NOT IN (SELECT student_id FROM group_members WHERE term_id=?)", [body.termId]),
