...
function StageWorkspace({ meetingId, initial, reload }: { meetingId: string; initial: Payload; reload: () => Promise<unknown> }) {
  ...
  return <div className="stage-workspace">
    ...
    {current.version ? <div className="sticky-actions"><span>현재 수정 내용을 새 버전으로 저장합니다. 확정 시 수정 내용도 함께 저장됩니다.</span>{message ? <p role="status" aria-live="polite">{message}</p> : null}<button className="button button-secondary" disabled={busy || !leaderId} onClick={() => saveDraft()}>{busy ? "처리 중…" : "수동 수정본 저장"}</button><button className="button button-primary" disabled={busy || !leaderId} onClick={confirmPlan}>{busy ? "처리 중…" : "등단표 확정"}</button></div> : null}
